...
export interface PaginatorOptions<T extends unknown[] = []> extends Omit<CollectorOptions<T>, 'dispose'> {
	...
	message: Message;
	/**
	 * The actual pages of this paginator.
	 */
	pages: PaginatorPage[];
	/**
 ...
 * @typeparam V The initial value to handle.
 * @typeparam T Extra arguments after the initial value, if any.
 */
export abstract class Paginator<V, T extends unknown[] = []>  {
	...
	public constructor(options: PaginatorOptions<T>) {
		...
		this.timeout = options.time ?? 60000;
	 ...
	 */
	public abstract collect(value: V): Awaited<void>;
	 ...
	 */
	public abstract end(collected: Collector<Snowflake, V, T>['collected'], reason: string): Awaited<void>;
}